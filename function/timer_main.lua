	...
	if isPlaying() and self and self.Enable then
		...
		if not self.Timer_Enable and not managers.groupai:state():whisper_mode() then
			...
			for _, unit in pairs(World:find_units_quick("all", 1)) do
				...
				for k, v in pairs (self.Unit_Remove_When_Loud and self.Unit_Remove_When_Loud[self.Enable] or {}) do
		...
		if self.Timer_Enable and self.Delay_Timer < _nowtime and self.Go_Loud_Stage == 1 then
			...
			if _enemy_type_amount["sniper"] < self._Spawning_Other_Total["sniper"][_D] then
				...
				local _pos_sniper = self.Spawning_Other.sniper[self.Enable] or self.Spawning_Other.sniper.pos
				self:_full_function_spawn(Idstring("units/pd2_dlc_drm/characters/ene_zeal_swat_heavy_sniper/ene_zeal_swat_heavy_sniper"), _pos_sniper[math.random(#_pos_sniper)], Rotation(0, 0, 1))
				if _D == "sm_wish" then
					self:_full_function_spawn(Idstring("units/pd2_dlc_drm/characters/ene_zeal_swat_heavy_sniper/ene_zeal_swat_heavy_sniper"), _pos_sniper[math.random(#_pos_sniper)], Rotation(0, 0, 1))
				end
			end
